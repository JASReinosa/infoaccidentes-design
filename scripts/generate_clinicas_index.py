...
SOURCE_FILE = "clinicas.html"
TARGET_DIR = "clinicas-accidentes-trafico"
...
def generate_clinicas_index():
    ...
    # 1. Update relative paths for depth change (Root -> /clinicas-accidentes-trafico/)
    ...
    <section id="directorio" class="py-20 bg-white border-t border-slate-200">
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
            <div class="mb-12 text-center">
                <h2 class="text-3xl font-bold tracking-tight text-text-dark">Directorio Completo de Clínicas</h2>
                <p class="mt-2 text-text-muted">Encuentra centros de rehabilitación en tu localidad</p>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
    """
    
    for name, slug in cities:
        # Link is relative to clinicas-accidentes-trafico/index.html, so "./slug/" or "slug/"
        link = f"{slug}/" 
        directory_html += f"""
                <a href="{link}" class="group flex items-center p-3 rounded-lg hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                    ...
                    <span class="text-sm font-medium text-text-dark group-hover:text-primary">{name}</span>
                </a>
        """
    ...
    # 6. Update "Ver todas las provincias" link
    # The button text is "Ver todas las provincias"
    target_link = None
    for a in soup.find_all("a"):
        if "Ver todas las provincias" in a.get_text():
            target_link = a
            break
            
    if target_link:
        target_link['href'] = "#directorio"
        print("Updated 'Ver todas las provincias' link target.")
    else:
        print("Warning: 'Ver todas las provincias' link not found.")
